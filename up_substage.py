...
def main(substage_id, qdict):
    ...
    q = 'SELECT id, stage_id, seqnum FROM substages WHERE stage_id=%s AND seqnum<%s ORDER BY seqnum DESC'
    ...
    if len(rows) > 0:
        ...
        c.execute(q, (prev_seqnum, substage_id))
        q = 'UPDATE substages SET seqnum=%s WHERE id=%s'
        c.execute(q, (seqnum, prev_substage_id))
        cnx.commit()
